...
void JMRI_HTML::html_init() {
        ...
        if (LittleFS.exists("/indexx.html") ) {
            JMRI_HELPER::logging(2,"HTML file present...");
        } else {   

              JMRI_HELPER::logging(1,"Attempting to format SPIFFS...");
               
              if(LittleFS.format()){
                      JMRI_HELPER::logging(1,"File System Formatted...\n");
              } else {
                      JMRI_HELPER::logging(1,"File System Formatting Error...\n");
              }
              
              File f = LittleFS.open("/index.html", "w");   
              //copy content of the PROGMEM var to SPIFFS(LittleFS) file  
              f.printf_P(indexhtml);
              JMRI_HELPER::logging(2,"index.html file created with size %d ...\n", f.size());
              f.close();
        }
        ...
        if (LittleFS.exists("/jmri_jsx.js") ) {
              JMRI_HELPER::logging(2,"jmri_js.js file present...\n");
        } else {   
              
              File fc = LittleFS.open("/jmri_js.js", "w");
              //copy content of the PROGMEM var to SPIFFS(LittleFS) file       
              fc.printf_P(jscript);        
              JMRI_HELPER::logging(2,"javascript file created with size %d ...\n", fc.size());
              fc.close();
       ...
       //check if logbox.html exist in SPIFFS(LittleFS), if not create it.
        ...
        if (LittleFS.exists("/loggingx.html") ) {
              JMRI_HELPER::logging(2,"logbox.html file present...\n");
        } else {   
              
              File fc = LittleFS.open("/logging.html", "w");
              //copy content of the PROGMEM var to SPIFFS(LittleFS) file       
              fc.printf_P(logbox);          
              JMRI_HELPER::logging(2,"logging.html file created with size %d ...\n", fc.size());
              fc.close();
        ...
        //check if style.css exist in SPIFFS(LittleFS), if not create it.
        //see style.css for the content of index.html
        if (LittleFS.exists("/stylex.css") ) {
              JMRI_HELPER::logging(2,"style.css file present...\n");
        } else {   
              
              File fc = LittleFS.open("/style.css", "w");
              //copy content of the PROGMEM var to SPIFFS(LittleFS) file       
              fc.printf_P(stylecss);         
              JMRI_HELPER::logging(2,"CSS file created with size %d ...\n", fc.size());
              fc.close();
